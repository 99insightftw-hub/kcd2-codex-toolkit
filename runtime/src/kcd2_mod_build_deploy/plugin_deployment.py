"""Deterministic packaging and atomic, receipt-backed mod-build-deploy deployment."""
...
CONTRACT = "kcd2.mod-build-deploy-plugin-deployment.v1"
...
def _validate_recipe(recipe: Mapping[str, Any]) -> tuple[str, list[Mapping[str, Any]]]:
    if recipe.get("contract") != CONTRACT or recipe.get("schema_version") != "1.0":
        raise PluginDeploymentError("mod-build-deploy deployment recipe contract is invalid")
...
def _catalog_plugin(path: Path) -> Mapping[str, Any]:
    ...
    if len(matches) != 1:
        raise PluginDeploymentError("catalog must contain exactly one mod-build-deploy entry")
    return matches[0]
...
def verify_installed_catalog(
    ...
    return {
        "schema_version": "kcd2.mod-build-deploy-catalog-verification.v1",
        "status": "PASS",
...
def install_plugin_atomic(
    ...
    *,
    ...
    try:
        ...
        receipt_payload = {
            "schema_version": "kcd2.mod-build-deploy-install-receipt.v1",
            "status": "installed",
        ...
        failure_payload = {
            "schema_version": "kcd2.mod-build-deploy-install-failure-receipt.v1",
...
def restore_plugin_atomic(install_receipt_path: Path | str) -> dict[str, Any]:
    ...
    try:
        ...
        payload = {
            "schema_version": "kcd2.mod-build-deploy-restore-receipt.v1",
            "status": "restored",
            "transaction_id": transaction_id,
            "restored_tree_sha256": restored_tree,
            "restored_catalog_sha256": restored_catalog,
